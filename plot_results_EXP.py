#!/usr/bin/env python3
"""
Plot EXP test results from random_cases.csv
...
Options:
    ...
    --output FILE       Output image file (default: build/exp_plot.png)
...
def main():
    parser = argparse.ArgumentParser(
        description='Plot EXP test results',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__
    )
    parser.add_argument('--input', default='build/random_cases.csv',
                        help='Input CSV file (default: build/random_cases.csv)')
    parser.add_argument('--output', default='build/exp_plot.png',
                        help='Output image file (default: build/exp_plot.png)')
    ...
    plt.title(f'EXP Function - {", ".join([style_map.get(c, {}).get("label", c) for c in curves])}',
              fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.legend(loc='best', fontsize=10)

    # Add reference exp curve
    x_ref = np.linspace(df['in'].min(), df['in'].max(), 1000)
    y_ref = np.exp(x_ref)
    plt.plot(x_ref, y_ref, 'k--', alpha=0.3, linewidth=1, label='Ideal exp(x)')
    plt.legend(loc='best', fontsize=10)
